...
def get_code_version(short_sha=True):
    ...
    try:
        # 修复1：指定编码为系统默认编码（Windows为gbk，Linux/Mac为utf-8）
        sha = check_output('git rev-parse HEAD', stderr=STDOUT,
                         shell=True, encoding='utf-8' if os.name != 'nt' else 'gbk')
        if short_sha:
            sha = sha[:7]
        return sha.strip()  # 去除换行符
    except CalledProcessError:
        # 修复2：用Python内置方法获取当前路径，替代系统pwd命令
        pwd = os.getcwd()
        print(f'Working dir {pwd} is not a git repo.')
        return "unknown"  # 返回默认值，避免后续拼接字符串报错
    except UnicodeDecodeError:
        # 兜底：若仍有编码问题，返回默认值
        pwd = os.getcwd()
        print(f'Encoding error when get git version, working dir: {pwd}')
        return "unknown"
