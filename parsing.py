...
# 移除错误的 from typing import ArgumentParser 导入（ArgumentParser 来自 argparse）


def add_kapt_args(parser: ArgumentParser):
    """添加 KAPT 提示模块的参数"""
    ...
    # 每个任务的提示数量
    parser.add_argument(
        '--num_prompts_per_task',
        type=int,
        default=10,
        help='动态提示池中每个任务的提示候选数量'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='当前下游任务的ID (0-13 对应 MoleculeNet 的14个任务)'
    )
...
def parse_train_args() -> Namespace:
    ...
    add_kapt_args(parser)  # 新增：添加 KAPT 相关参数到训练参数解析
    args = parser.parse_args()
    modify_train_args(args)  # 修复：添加参数验证步骤
